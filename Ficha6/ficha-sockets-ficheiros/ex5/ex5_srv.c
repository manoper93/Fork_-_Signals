    ...
    print_address(&clt_addr, addrlen);
		...
		if (pid == 0){ 						//filho
			...
			char num_bytes[4];
				...
				int nbytes = read(new_socket_descriptor, &buffer, sizeof(buffer));
				if (nbytes == -1) {
					perror("error write\n");
				...
				for(int i=0; i < atoi(num_bytes)+1; i++){
					sscanf(&buffer[num_bytes_str+i], "%s", &msg1.text[i]);
				}
				
				printf("Numero de Aluno= %.7s\n", msg1.student_id);
				printf("Numero de bytes da mensagem= %s\n", num_bytes);
				printf("Mensagem= %s\n", msg1.text);
				...
				for (int i = 0; i < atoi(num_bytes); i++) {
