...
/* 1.1)
 * 
Identificador do processo: 3894
Nova thread criada (tid1), fazendo a chamada à função mythread(buf) 
Nova thread criada (tid2), fazendo a chamada à função mythread(buf) 
Pausa de 5 segundos na thread principal
33889944::  OOllae  +-+-+-+--+-++-+-+-+--++-+-+-+-+-+--+-+-+-+-+-+-+-+-+-+-+-+-+Thread principal vai aguardar que a thread identificada por tid1 termine.
-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-++-+-+-+-+-+--+-+-++-

 * 1.2)
 * 
Espera que o thread termine

 * 1.3)
 * 
Aloca um local de memoria a cada sprintf.
Copia o ultimo sprintf no buf.
